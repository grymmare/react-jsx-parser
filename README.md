...
However, the component **_does_** support inline arrow functions with a simple statement, such as: 
 - Works: `{ [1,2].map( value => <span>{value}</span> ) }`
 - Does not work: `{ [1,2].map( value => { return <span>{value}</span>; } ) }`
